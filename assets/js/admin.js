/* Vetra RebarCut 2.5 — admin/front-end SPA */
/* global VCO */
(function () {
    'use strict';

    var SIZES = VCO.sizes || [8, 10, 12, 14, 16, 18, 20, 22, 25, 28, 32];
    var PAPERS = ['A4', 'A3', 'A5', 'B5', 'Letter', 'Legal', 'A2'];
    var state = {
        projects: [], pid: 0, project: null, cuts: [], inventory: [], autoInventory: [],
        cfg: { min_reusable_m: 0.5, scrap_ratio: 0.3, kg_price: 0 },
        result: null, tab: 'cuts', paper: 'A4', dirty: false
    };
    ...
    function paperSelect() {
        return '<select id="vco-paper" class="vco-paper" title="اندازه کاغذ چاپ">' +
            PAPERS.map(function (p) { return '<option value="' + p + '"' + (state.paper === p ? ' selected' : '') + '>' + p + '</option>'; }).join('') + '</select>';
    }
    function bindPaper() {
        if (el('vco-paper')) { el('vco-paper').onchange = function () { state.paper = this.value; }; }
    }
    function applyPaper(size) {
        var s = document.getElementById('vco-page-style');
        if (!s) { s = document.createElement('style'); s.id = 'vco-page-style'; document.head.appendChild(s); }
        s.textContent = '@page { size: ' + (size || 'A4') + ' portrait; margin: 10mm; }';
    }
    function printPage() { applyPaper(state.paper); setTimeout(function () { window.print(); }, 50); }
    ...
    function mapCut(c) { return { size: parseInt(c.size, 10) || 12, length: num(c.length), quantity: parseInt(c.quantity, 10) || 1, label: c.label || '', note: c.note || '' }; }
    function mapInv(c) { return { size: parseInt(c.size, 10) || 12, bar_length: num(c.bar_length), quantity: parseInt(c.quantity, 10) || 1, location: c.location || '' }; }
    function normalizeCfg(cfg) { cfg = cfg || {}; return { min_reusable_m: num(cfg.min_reusable_m, 0.5), scrap_ratio: num(cfg.scrap_ratio, 0.3), kg_price: num(cfg.kg_price !== undefined ? cfg.kg_price : (cfg.default_price ? cfg.default_price / 1000 : 0), 0) }; }

    var TABS = [
        { id: 'cuts', name: '✂️ لیست برش' },
        { id: 'inv', name: '🏗️ انبار' },
        { id: 'settings', name: '⚙️ تنظیمات' },
        { id: 'report', name: '📊 نقشه گرافیکی' },
        { id: 'table', name: '🧾 جدول برش' },
        { id: 'labels', name: '🏷️ چاپ لیبل' }
    ];
    ...
    function render() {
        ...
        if (state.tab === 'cuts') { renderCuts(area); }
        else if (state.tab === 'inv') { renderInventory(area); }
        else if (state.tab === 'settings') { renderSettings(area); }
        else if (state.tab === 'table') { renderTable(area); }
        else if (state.tab === 'labels') { renderLabels(area); }
        else { renderReport(area); }
    }
    function saveBar(actions) { return '<div class="vco-actions-row vco-no-print">' + actions + '</div>'; }

    function renderCuts(area) {
        if (!state.cuts.length) { state.cuts.push({ size: 12, length: 0, quantity: 0, label: '', note: '' }); }
        var rows = state.cuts.map(function (c, i) { return '<tr><td class="vco-num">' + fa(i + 1) + '</td><td>' + select('size', i, SIZES, c.size, 'Ø') + '</td>' +
        ...
        area.innerHTML = '<h3>لیست برش — گروه‌بندی خودکار بر اساس سایز</h3>' + saveBar('<button class="btn vco-btn" id="vco-addrow">+ ردیف</button><button class="btn vco-btn-outline" id="vco-paste">📋 کپی از Excel</button><button class="btn vco-btn-outline" id="vco-clearrows">پاک کردن</button><button class="btn vco-btn-green" id="vco-savecuts">💾 ذخیره لیست</button>') +
            '<div class="vco-table-wrap"><table class="vco-table"><thead><tr><th>ردیف</th><th>سایز *</th><th>طول برش (m) *</th><th>تعداد *</th><th>لیبل</th><th>توضیحات</th><th></th></tr></thead><tbody>' + rows + '</tbody></table></div>' +
            '<p class="vco-hint">ترتیب ستون‌های Excel: سایز، طول، تعداد، لیبل، توضیحات.</p>';
        bindTable(area, state.cuts);
        el('vco-addrow').onclick = function () { state.cuts.push({ size: 12, length: 0, quantity: 1, label: '', note: '' }); state.dirty = true; render(); };
    ...
    function openPasteModal(target) {
        ...
        back.innerHTML = '<div class="vco-modal"><h3>درون‌ریزی از Excel</h3><div class="vco-hint">' + (target === 'cuts' ? 'ستون‌ها: سایز | طول برش | تعداد | لیبل | توضیحات' : 'ستون‌ها: سایز | طول شاخه | تعداد | محل') + '</div><textarea id="vco-paste-data" dir="rtl"></textarea><div class="vco-actions-row"><button class="btn" id="vco-paste-cancel">انصراف</button><button class="btn vco-btn" id="vco-paste-ok">افزودن</button></div></div>';
    ...
    function parseTable(text, target) {
        var out = []; text.split(/\r?\n/).forEach(function (line) { line = line.trim(); if (!line) { return; } var c = line.split(/\t|;|،| {2,}/).map(function (x) { return x.trim(); }); var size = parseInt(c[0], 10); if (isNaN(size) || c.length < 3) { return; }
            if (target === 'cuts') { out.push({ size: size, length: num(c[1]), quantity: parseInt(c[2], 10) || 1, label: c[3] || '', note: c[4] || '' }); }
            else { out.push({ size: size, bar_length: num(c[1]), quantity: parseInt(c[2], 10) || 1, location: c[3] || '' }); }
        }); return out;
    }

    function renderInventory(area) {
        var rows = state.inventory.map(function (c, i) { return '<tr><td class="vco-num">' + fa(i + 1) + '</td><td>' + select('size', i, SIZES, c.size, 'Ø') + '</td><td><input type="number" step="0.01" min="0" data-f="bar_length" data-i="' + i + '" value="' + c.bar_length + '"></td><td><input type="number" step="1" min="0" data-f="quantity" data-i="' + i + '" value="' + c.quantity + '"></td><td><input type="text" data-f="location" data-i="' + i + '" value="' + esc(c.location) + '"></td><td><button class="btn vco-btn-red vco-btn-sm" data-del="' + i + '">✕</button></td></tr>'; }).join('');
        var auto = state.autoInventory.length ? '<h4>تکه‌های قابل استفاده افزوده‌شده خودکار</h4><div class="vco-auto-offcuts">' + state.autoInventory.map(function (c) { return '<span>Ø' + c.size + ' — ' + fa(c.bar_length, 2) + 'm × ' + fa(c.quantity) + '</span>'; }).join('') + '</div>' : '<div class="vco-hint">پس از محاسبه، ته‌مانده‌های بزرگ‌تر از حد مجاز به‌صورت خودکار در این بخش ثبت می‌شوند.</div>';
        area.innerHTML = '<h3>انبار — موجودی همیشه در محاسبه مصرف می‌شود</h3>' + saveBar('<button class="btn vco-btn" id="vco-addinv">+ ردیف</button><button class="btn vco-btn-outline" id="vco-paste-inv">📋 کپی از Excel</button><button class="btn vco-btn-green" id="vco-saveinv">💾 ذخیره موجودی</button>') + '<div class="vco-table-wrap"><table class="vco-table"><thead><tr><th>ردیف</th><th>سایز *</th><th>طول شاخه (m) *</th><th>تعداد *</th><th>محل/توضیح</th><th></th></tr></thead><tbody>' + (rows || '<tr><td colspan="6" class="vco-empty">موجودی دستی ثبت نشده</td></tr>') + '</tbody></table></div>' + auto;
        bindTable(area, state.inventory); el('vco-addinv').onclick = function () { state.inventory.push({ size: 12, bar_length: 6, quantity: 1, location: '' }); state.dirty = true; render(); }; el('vco-paste-inv').onclick = function () { openPasteModal('inventory'); }; el('vco-saveinv').onclick = function () { saveInventory().then(function (r) { toast('موجودی ذخیره شد (' + r.saved + ' ردیف)'); }); };
    }

    function renderSettings(area) {
        var p = state.project, stockSel = [6, 12].indexOf(num(p.stock_length)) > -1 ? String(p.stock_length) : 'custom';
        area.innerHTML = '<h3>تنظیمات محاسبه</h3><div class="vco-info">موجودی انبار در هر محاسبه به‌صورت خودکار اولویت دارد. قیمت به‌صورت ریال بر کیلوگرم وارد می‌شود.</div><div class="vco-form-grid">' +
            ...
            field('نسبت ارزش ضایعات (۰ تا ۱)', '<input type="number" step="0.05" min="0" max="1" id="vco-scrap" value="' + state.cfg.scrap_ratio + '">') + field('قیمت هر کیلوگرم (ریال)', '<input type="number" step="1000" min="0" id="vco-price" value="' + state.cfg.kg_price + '">') + '</div>' + saveBar('<button class="btn vco-btn-green" id="vco-savesettings">💾 ذخیره تنظیمات</button>');
    ...
    function renderReport(area) {
        ...
        var html = '<div class="vco-no-print vco-actions-row">' + paperSelect() + '<button class="btn vco-btn" id="vco-print">🖨 چاپ گرافیک / PDF</button><button class="btn vco-btn-outline" id="vco-table-link">🧾 جدول برش</button><button class="btn vco-btn-outline" id="vco-labels">🏷 چاپ لیبل‌ها</button><a class="btn vco-btn-outline" href="' + csvLink() + '">⬇ Excel (CSV)</a><button class="btn vco-btn-outline" id="vco-json">⬇ JSON</button></div>';
        ...
        html += '<h4>📦 لیست خرید پس از کسر موجودی</h4><div class="vco-table-wrap"><table class="vco-table"><thead><tr><th>سایز</th><th>طول شاخه</th><th>تعداد</th><th>وزن (kg)</th><th>هزینه (ریال)</th></tr></thead><tbody>' + (r.purchase_list.length ? r.purchase_list.map(function (p) { return '<tr><td>Ø' + p.size + '</td><td>' + fa(p.bar_length, 1) + 'm</td><td>' + fa(p.qty_bars) + '</td><td>' + fa(p.weight_kg, 1) + '</td><td>' + fa(p.cost) + '</td></tr>'; }).join('') : '<tr><td colspan="5" class="vco-empty">نیازی به خرید شاخه نو نیست ✓</td></tr>') + '</tbody></table></div>';
        ...
        r.groups.forEach(function (g) { var s = g.stat; html += '<div class="vco-group-block"><div class="vco-group-title">Ø' + s.size + '</div><div class="vco-group-meta">قطعات: ' + fa(s.pieces_count) + ' | نو: ' + fa(s.new_bars) + ' | انبار: ' + fa(s.inventory_bars) + ' (' + fa(s.inventory_len, 1) + 'm) | ضایعات: ' + fa(s.waste_length, 2) + 'm</div>';
            if (s.errors && s.errors.length) { html += '<div class="vco-errors">⚠ ' + s.errors.map(esc).join('<br>⚠ ') + '</div>'; }
            g.bars.forEach(function (b, bi) { var cap = b.type === 'new' ? 'شاخه نو ' + fa(b.length, 1) + 'm — شماره ' + fa(bi + 1) : 'از انبار ' + fa(b.length, 1) + 'm' + (b.location ? ' | ' + esc(b.location) : ''); html += '<div class="vco-bar-row"><div class="vco-bar-caption">' + cap + ' — Ø' + s.size + '</div><div class="vco-bar">' + barSegments(b, s.size) + '</div></div>'; });
            html += '</div>'; });
        area.innerHTML = html;
        el('vco-print').onclick = printPage; el('vco-table-link').onclick = function () { state.tab = 'table'; render(); }; el('vco-labels').onclick = function () { state.tab = 'labels'; render(); }; el('vco-json').onclick = function () { downloadFile('vetra-cut-report-' + state.pid + '.json', JSON.stringify(r, null, 2), 'application/json'); };
        bindPaper();
    }
    function stat(label, value, kind) { return '<div class="vco-stat ' + (kind || '') + '"><b>' + esc(value) + '</b><span>' + label + '</span></div>'; }
    function barSegments(b, size) { var html = '', L = b.length; b.pieces.forEach(function (p) { var w = p.length / L * 100, tip = fa(p.length, 2) + 'm' + (p.label ? ' — ' + p.label : '') + ' — Ø' + size; html += '<div class="vco-seg vco-seg-piece" style="width:' + w + '%" title="' + esc(tip) + '">' + (w > 7 ? fa(p.length, 2) + 'm' : '') + (w > 13 && p.label ? ' ' + esc(p.label) : '') + '</div>'; }); if (b.reusable > 0.001) { html += '<div class="vco-seg vco-seg-reusable" style="width:' + b.reusable / L * 100 + '%">ته‌مانده ' + fa(b.reusable, 1) + '</div>'; } if (b.waste > 0.001) { html += '<div class="vco-seg vco-seg-waste" style="width:' + b.waste / L * 100 + '%">ضایعات</div>'; } return html; }

    /* ---------- static table tab ---------- */
    function renderTable(area) {
        if (!state.result) { area.innerHTML = '<div class="vco-empty">ابتدا محاسبه را اجرا کنید.</div>'; return; }
        var r = state.result;
        var bars = barSummary(r);
        var html = '<div class="vco-no-print vco-actions-row">' + paperSelect() + '<button class="btn vco-btn" id="vco-print-table">🖨 چاپ / PDF</button><a class="btn vco-btn-outline" href="' + csvLink() + '">⬇ Excel (CSV)</a><button class="btn vco-btn-outline" id="vco-table-json">⬇ JSON</button></div>';
        html += '<h3>خلاصه شاخه‌های مورد نیاز</h3><div class="vco-table-wrap"><table class="vco-table"><thead><tr><th>ردیف</th><th>سایز</th><th>طول شاخه (m)</th><th>نوع</th><th>تعداد شاخه</th></tr></thead><tbody>' +
            (bars.length ? bars.map(function (b, i) { return '<tr><td class="vco-num">' + fa(i + 1) + '</td><td>Ø' + b.size + '</td><td>' + fa(b.length, 1) + '</td><td>' + b.type + '</td><td>' + fa(b.qty) + '</td></tr>'; }).join('') : '<tr><td colspan="5" class="vco-empty">داده‌ای نیست</td></tr>') +
            '</tbody></table></div>';
        html += '<h3 style="margin-top:22px">لیست برش بهینه‌شده</h3><div class="vco-table-wrap"><table class="vco-table"><thead><tr><th>ردیف</th><th>طول برش (m)</th><th>سایز</th><th>لیبل</th><th>توضیحات</th><th>شاخه</th></tr></thead><tbody>';
        var rn = 0;
        r.groups.forEach(function (g) {
            ...
            g.bars.forEach(function (b) {
                ...
                b.pieces.forEach(function (p) {
                    rn++;
                    html += '<tr><td class="vco-num">' + fa(rn) + '</td><td>' + fa(p.length, 2) + '</td><td>Ø' + g.stat.size + '</td><td>' + esc(p.label || '—') + '</td><td>' + esc(p.note || '—') + '</td><td>' + fa(bi) + '</td></tr>';
                });
            });
        });
        html += (rn ? '' : '<tr><td colspan="6" class="vco-empty">داده‌ای نیست</td></tr>') + '</tbody></table></div>';
        area.innerHTML = html;
        el('vco-print-table').onclick = printPage;
        el('vco-table-json').onclick = function () { downloadFile('vetra-cut-table-' + state.pid + '.json', JSON.stringify({ bars: bars, report: r }, null, 2), 'application/json'); };
        bindPaper();
    }
    function barSummary(r) {
        ...
        r.groups.forEach(function (g) {
            g.bars.forEach(function (b) {
                var key = g.stat.size + '|' + b.length + '|' + b.type;
                if (!agg[key]) { agg[key] = { size: g.stat.size, length: b.length, type: b.type === 'new' ? 'شاخه نو' : 'انبار', qty: 0 }; order.push(key); }
                agg[key].qty++;
            });
        });
    ...
    function labelItems(expand) {
        ...
        if (r && r.groups) { r.groups.forEach(function (g) { g.bars.forEach(function (b, bi) { b.pieces.forEach(function (p) { items.push({ size: g.stat.size, length: p.length, label: p.label || ('قطعه Ø' + g.stat.size), note: p.note || '', source: b.type === 'new' ? 'شاخه نو' : 'انبار', bar: bi + 1 }); }); }); }); }
        if (!items.length) { state.cuts.forEach(function (c) { for (var i = 0; i < c.quantity; i++) { items.push({ size: c.size, length: c.length, label: c.label || ('قطعه Ø' + c.size), note: c.note || '', source: 'لیست برش', bar: '-' }); } }); }
        if (expand) { return items.slice(0, 2000); }
        var map = {}, out = []; items.forEach(function (x) { var k = x.size + '|' + x.length + '|' + x.label; if (!map[k]) { map[k] = { size: x.size, length: x.length, label: x.label, source: x.source, bar: x.bar, count: 0 }; out.push(map[k]); } map[k].count++; }); return out;
    }
    function renderLabels(area) {
        var items = labelItems(false);
        area.innerHTML = '<div class="vco-label-head"><h3>لیبل قطعات</h3><p>برای لیبل‌پرینتر یا پرینتر معمولی، اندازه کاغذ و مقیاس را در پنجره چاپ انتخاب کنید.</p></div>' +
            saveBar(paperSelect() + '<label class="vco-check"><input type="checkbox" id="vco-label-expand"> چاپ یک لیبل برای هر قطعه</label><button class="btn vco-btn" id="vco-print-labels">🖨 چاپ لیبل‌ها</button>') +
            '<div class="vco-label-preview">' + items.map(function (x) { return '<div class="vco-label-card"><strong>Ø' + x.size + '</strong><b>' + fa(x.length, 2) + ' m</b><span>' + esc(x.label) + '</span><small>' + esc(x.source) + ' | تعداد: ' + fa(x.count) + '</small></div>'; }).join('') + '</div>';
        el('vco-print-labels').onclick = function () { printLabels(el('vco-label-expand').checked); };
        bindPaper();
    }
    function printLabels(expand) {
        var items = labelItems(expand);
        if (!items.length) { toast('لیبلی برای چاپ وجود ندارد', 'err'); return; }
        var w = window.open('', '_blank');
        if (!w) { toast('پنجره چاپ توسط مرورگر مسدود شد', 'err'); return; }
        var title = state.project ? state.project.name : 'Vetra RebarCut';
        var paper = state.paper || 'A4';
        w.document.write('<!doctype html><html dir="rtl"><head><meta charset="utf-8"><title>لیبل - ' + esc(title) + '</title><style>@page{size:' + paper + ' portrait;margin:5mm}*{box-sizing:border-box}body{font-family:Tahoma,Arial;direction:rtl;margin:0}.sheet{display:grid;grid-template-columns:repeat(2,90mm);gap:4mm}.label{width:90mm;min-height:42mm;border:1px solid #111;padding:4mm;display:flex;flex-direction:column;justify-content:space-between;page-break-inside:avoid}.project{font-size:10pt}.group{font-size:14pt;font-weight:bold}.length{font-size:20pt;font-weight:bold;direction:ltr;text-align:right}.code{font-size:13pt;overflow-wrap:anywhere}.note{font-size:10pt;color:#333}.meta{font-size:9pt;color:#444}</style></head><body><div class="sheet">');
        items.forEach(function (x, i) {
            w.document.write('<div class="label"><span class="project">' + esc(title) + '</span><span class="group">Ø' + x.size + '</span><span class="length">' + Number(x.length).toFixed(2) + ' m</span><span class="code">' + esc(x.label) + '</span>' + (x.note ? '<span class="note">' + esc(x.note) + '</span>' : '') + '<span class="meta">' + esc(x.source) + ' | شاخه: ' + esc(x.bar) + ' | #' + (i + 1) + '</span></div>');
        });
        w.document.write('</div></body></html>'); w.document.close(); w.focus(); setTimeout(function () { w.print(); }, 350);
    }
